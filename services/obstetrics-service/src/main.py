"""Obstétrique (🤰) — Module 10 MEDISUITE.

Biométrie fœtale, CTG FIGO, clarté nucale, pré-éclampsie, DG IADPSG, Bishop.
...
app: FastAPI = create_service_app(
    "obstetrics-service", "Obstétrique", "Biométrie fœtale, CTG FIGO, clarté nucale, pré-éclampsie, DG IADPSG, Bishop.",
    module_label="Module 10 · Obstétrique")

engine = engine_for("obstetrics-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Consultation T3 — 32 SA', 'pat0022', 'YEBOAH Mariam', 'CTG rassurant'), ('HGPN2 — HG déclenchement', 'pat0023', 'BINTOU Aissata', 'Bishop défavorable'), ('HGPN1 — charge glycémique', 'pat0024', 'SERY Marthe', 'HGPO 100 g')]
...
def module_info() -> dict:
    return {"module": 10, "nom": "Obstétrique", "service": "obstetrics-service",
            "scores": ['bishop', 'dg-iadpsg'],
...
@app.post("/api/v1/scores/bishop", tags=["scores cliniques"])
def score_bishop(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score bishop — medisuite_rules.gyne_obstetrique"""
    try:
        result = gyne_obstetrique.bishop(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "bishop", "resultat": result,
            "moteur": "medisuite_rules.gyne_obstetrique"}


@app.post("/api/v1/scores/dg_iadpsg", tags=["scores cliniques"])
def score_dg_iadpsg(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score dg-iadpsg — module courant"""
    try:
        result = _gdm_iadpsg(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "dg_iadpsg", "resultat": result,
