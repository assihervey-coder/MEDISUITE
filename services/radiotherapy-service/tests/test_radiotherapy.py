"""Tests radiotherapy-service : santé, module-info, scores, cas cliniques."""
import pathlib
import sys

ROOT = pathlib.Path(__file__).resolve().parents[3]
for p in ("packages/medisuite-core", "packages/clinical-rules",
          str(ROOT / "services" / "radiotherapy-service" / "src")):
    sys.path.insert(0, p)
...
def test_module_info():
    ...
    assert info["module"] == 23 and "radiotherapy-service" in info["service"]
...
def test_score_eqd2():
    r = client.post("/api/v1/scores/eqd2", json={'dose_par_seance': 2.0, 'nb_seances': 30, 'alpha_beta': 3}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide


def test_score_ecog():
    r = client.post("/api/v1/scores/ecog", json={'karnofsky': 60}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide
