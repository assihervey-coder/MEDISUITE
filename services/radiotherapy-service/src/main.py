"""Radiothérapie (🎯) — Module 23 MEDISUITE.

Contours GTV/CTV/PTV, IMRT/VMAT, DVH, gamma index, EQD2 radiobiologie, QA.
...
app: FastAPI = create_service_app(
    "radiotherapy-service", "Radiothérapie", "Contours GTV/CTV/PTV, IMRT/VMAT, DVH, gamma index, EQD2 radiobiologie, QA.",
    module_label="Module 23 · Radiothérapie")

engine = engine_for("radiotherapy-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Plan VMAT prostate SIB', 'pat0061', 'DOUMBIA Sira', '78 Gy/39 fx'), ('SRS métastase unique', 'pat0062', 'KONE Salia', '24 Gy/1 fx'), ('Repositionnement IGRT', 'pat0063', 'YEO Flavie', 'CBCT quotidien')]
...
def module_info() -> dict:
    return {"module": 23, "nom": "Radiothérapie", "service": "radiotherapy-service",
            "scores": ['eqd2', 'ecog'],
...
@app.post("/api/v1/scores/eqd2", tags=["scores cliniques"])
def score_eqd2(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score eqd2 — module courant"""
    try:
        result = _eqd2(**body)
    ...
    return {"score_endpoint": "eqd2", "resultat": result,
