"""Urologie (🚹) — Module 21 MEDISUITE.

Prostate (PI-RADS, Gleason ISUP), BPH IPSS, calculs (SWL/URS), néphrométrie RENAL.
...
app: FastAPI = create_service_app(
    "urology-service", "Urologie", "Prostate (PI-RADS, Gleason ISUP), BPH IPSS, calculs (SWL/URS), néphrométrie RENAL.",
    module_label="Module 21 · Urologie")

engine = engine_for("urology-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('RMN prostate cible', 'pat0055', 'BOGNINI Awa', 'PI-RADS 4'), ('SBAU — retentions', 'pat0056', 'SOUMANE Youssouf', 'IPSS'), ('Masse rénale droite', 'pat0057', 'NANDA Théodore', 'RENAL')]
...
def module_info() -> dict:
    return {"module": 21, "nom": "Urologie", "service": "urology-service",
            "scores": ['ipss', 'gleason', 'renal-score', 'pirads'],
...
@app.post("/api/v1/scores/ipss", tags=["scores cliniques"])
def score_ipss(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score ipss — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.ipss(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "ipss", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/gleason", tags=["scores cliniques"])
def score_gleason(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score gleason — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.gleason_grade_group(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "gleason", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/renal_score", tags=["scores cliniques"])
def score_renal_score(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score renal-score — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.renal_nephrometry(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "renal_score", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/pirads", tags=["scores cliniques"])
def score_pirads(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score pirads — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.pirads_lesions(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "pirads", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}
