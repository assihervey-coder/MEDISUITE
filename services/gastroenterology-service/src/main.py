"""Gastro-entérologie (🫄) — Module 17 MEDISUITE.

Polypes, LI-RADS, MICI (Mayo/CDEIS), RGO LA, ulcère Forrest, Child-Pugh/MELD.
...
app: FastAPI = create_service_app(
    "gastroenterology-service", "Gastro-entérologie", "Polypes, LI-RADS, MICI (Mayo/CDEIS), RGO LA, ulcère Forrest, Child-Pugh/MELD.",
    module_label="Module 17 · Gastro-entérologie")

engine = engine_for("gastroenterology-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Coloscopie — polype 8 mm', 'pat0043', 'EHY Koffi', 'Polypectomie'), ('Cirrhose décompensée', 'pat0044', 'AKPLOGAN Kossi', 'Child-Pugh + MELD'), ('Rectocolite hémorragique', 'pat0045', 'YEBOUA Arlette', 'Mayo 2')]
...
def module_info() -> dict:
    return {"module": 17, "nom": "Gastro-entérologie", "service": "gastroenterology-service",
            "scores": ['child-pugh', 'meld', 'mayo', 'forrest'],
...
@app.post("/api/v1/scores/child_pugh", tags=["scores cliniques"])
def score_child_pugh(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score child-pugh — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.child_pugh(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "child_pugh", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/meld", tags=["scores cliniques"])
def score_meld(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score meld — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.meld(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "meld", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/mayo", tags=["scores cliniques"])
def score_mayo(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score mayo — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.mayo_endoscopic(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "mayo", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}


@app.post("/api/v1/scores/forrest", tags=["scores cliniques"])
def score_forrest(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score forrest — medisuite_rules.gastro_rheuma_uro"""
    try:
        result = gastro_rheuma_uro.forrest(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "forrest", "resultat": result,
            "moteur": "medisuite_rules.gastro_rheuma_uro"}
