"""ORL (👂) — Module 19 MEDISUITE.

Audiogramme PTA OMS, BPPV (Epley), sinusite Lund-Mackay, cancers ORL HPV.
...
app: FastAPI = create_service_app(
    "ent-service", "ORL", "Audiogramme PTA OMS, BPPV (Epley), sinusite Lund-Mackay, cancers ORL HPV.",
    module_label="Module 19 · ORL")

engine = engine_for("ent-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Vertige positionnel', 'pat0049', 'BIROKO Nadège', 'Dix-Hallpike'), ('Surdité perception', 'pat0050', 'ZONGO Ibrahim', 'PTA 4 fréquences'), ('Sinusite chronique TDM', 'pat0051', 'TRAORE Fatimetou', 'Lund-Mackay')]
...
def module_info() -> dict:
    return {"module": 19, "nom": "ORL", "service": "ent-service",
            "scores": ['pta-oms', 'lund-mackay', 'bppv'],
...
@app.post("/api/v1/scores/pta_oms", tags=["scores cliniques"])
def score_pta_oms(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score pta-oms — medisuite_rules.derma_ent_ophtalmo"""
    try:
        result = derma_ent_ophtalmo.pta_audiogramme(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "pta_oms", "resultat": result,
            "moteur": "medisuite_rules.derma_ent_ophtalmo"}


@app.post("/api/v1/scores/lund_mackay", tags=["scores cliniques"])
def score_lund_mackay(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score lund-mackay — medisuite_rules.derma_ent_ophtalmo"""
    try:
        result = derma_ent_ophtalmo.lund_mackay(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "lund_mackay", "resultat": result,
            "moteur": "medisuite_rules.derma_ent_ophtalmo"}


@app.post("/api/v1/scores/bppv", tags=["scores cliniques"])
def score_bppv(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score bppv — medisuite_rules.derma_ent_ophtalmo"""
    try:
        result = derma_ent_ophtalmo.bppv_interpretation(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "bppv", "resultat": result,
            "moteur": "medisuite_rules.derma_ent_ophtalmo"}
