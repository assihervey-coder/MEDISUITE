"""Diabétologie (🩸) — Module 06 MEDISUITE.

Risque ADA, HbA1c/TIR, complications (néphropathie via CKD-EPI), CGM Dexcom/Libre.
...
app: FastAPI = create_service_app(
    "diabetes-service", "Diabétologie", "Risque ADA, HbA1c/TIR, complications (néphropathie via CKD-EPI), CGM Dexcom/Libre.",
    module_label="Module 06 · Diabétologie")

engine = engine_for("diabetes-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('DT2 déséquilibré — HbA1c 9.8 %', 'pat0010', 'CISSÉ Bakary', 'Insulinotherapie à discuter'), ('Complication rénale — DFG', 'pat0011', 'SANGARÉ Rokia', 'Suivi néphrologique'), ('Éducation — comptage glucides', 'pat0012', "N'GUESSAN Franck", 'Atelier nutrition')]
...
def module_info() -> dict:
    return {"module": 6, "nom": "Diabétologie", "service": "diabetes-service",
            "scores": ['dfg-ckd-epi', 'imc-oms'],
...
@app.post("/api/v1/scores/imc_oms", tags=["scores cliniques"])
def score_imc_oms(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score imc-oms — medisuite_rules.scores"""
    try:
        ...
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "imc_oms", "resultat": result,
