"""Tests tumor-service : santé, module-info, scores, cas cliniques."""
import pathlib
import sys

ROOT = pathlib.Path(__file__).resolve().parents[3]
for p in ("packages/medisuite-core", "packages/clinical-rules",
          str(ROOT / "services" / "tumor-service" / "src")):
    sys.path.insert(0, p)
...
def test_module_info():
    ...
    assert info["module"] == 4 and "tumor-service" in info["service"]
...
def test_score_growth_rate():
    r = client.post("/api/v1/scores/growth_rate", json={'v1_ml': 12.0, 'v2_ml': 15.5}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide


def test_score_ecog():
    r = client.post("/api/v1/scores/ecog", json={'karnofsky': 80}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide
