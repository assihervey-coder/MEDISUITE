"""Tumeurs cérébrales (🧠) — Module 04 MEDISUITE.

Segmentation, volumétrie longitudinale, grading OMS 2021, planification RT.
...
app: FastAPI = create_service_app(
    "tumor-service", "Tumeurs cérébrales", "Segmentation, volumétrie longitudinale, grading OMS 2021, planification RT.",
    module_label="Module 04 · Tumeurs cérébrales")

engine = engine_for("tumor-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Gliome diffus — volumétrie M3', 'pat0004', 'YAO Serge', 'Croissance +29 %'), ('Métastases — comptage IRM', 'pat0005', 'AKA Awa', '3 lésions supra-tentorielles'), ('Suivi post-opératoire cavité', 'pat0006', 'BAMBA Karim', 'Stable')]
...
def module_info() -> dict:
    return {"module": 4, "nom": "Tumeurs cérébrales", "service": "tumor-service",
            "scores": ['growth-rate', 'ecog'],
...
@app.post("/api/v1/scores/growth_rate", tags=["scores cliniques"])
def score_growth_rate(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score growth-rate — module courant"""
    try:
        result = _growth_pct(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "growth_rate", "resultat": result,
