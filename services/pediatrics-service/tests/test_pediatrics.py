"""Tests pediatrics-service : santé, module-info, scores, cas cliniques."""
import pathlib
import sys

ROOT = pathlib.Path(__file__).resolve().parents[3]
for p in ("packages/medisuite-core", "packages/clinical-rules",
          str(ROOT / "services" / "pediatrics-service" / "src")):
    sys.path.insert(0, p)
...
def test_module_info():
    ...
    assert info["module"] == 15 and "pediatrics-service" in info["service"]
...
def test_score_apgar():
    r = client.post("/api/v1/scores/apgar", json={'items': [2, 2, 1, 2, 2]}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide


def test_score_imc_pediatre():
    r = client.post("/api/v1/scores/imc_pediatre", json={'imc_value': 17.2}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide
