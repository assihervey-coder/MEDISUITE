"""Pédiatrie (👶) — Module 15 MEDISUITE.

APGAR, courbes OMS, ictère néonatal, développement (Denver), calendrier PEV CI.
...
from medisuite_rules import scores

app: FastAPI = create_service_app(
    "pediatrics-service", "Pédiatrie", "APGAR, courbes OMS, ictère néonatal, développement (Denver), calendrier PEV CI.",
    module_label="Module 15 · Pédiatrie")

engine = engine_for("pediatrics-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Nouveau-né J1 — APGAR', 'pat0037', 'KOUADIO Bébé', '5 min'), ('Suivi croissance 18 mois', 'pat0038', 'BROU Grâce', 'Courbe OMS'), ('PEV — rappel vaccinal', 'pat0039', 'KOFFI Mickeal', 'Rougeole- Rubéole')]
...
def module_info() -> dict:
    return {"module": 15, "nom": "Pédiatrie", "service": "pediatrics-service",
            "scores": ['apgar', 'imc-pediatre'],
...
@app.post("/api/v1/scores/apgar", tags=["scores cliniques"])
def score_apgar(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score apgar — module courant"""
    try:
        result = _apgar(**body)
    ...
    return {"score_endpoint": "apgar", "resultat": result,
            "moteur": "medisuite_rules.custom"}


@app.post("/api/v1/scores/imc_pediatre", tags=["scores cliniques"])
def score_imc_pediatre(body: dict, user: dict = Depends(current_user)) -> dict:
    """Score imc-pediatre — medisuite_rules.scores"""
    try:
        result = scores.categorie_oms(**body)
    except TypeError as exc:
        raise HTTPException(422, f"paramètres invalides : {exc}")
    except ValueError as exc:
        raise HTTPException(422, str(exc))
    return {"score_endpoint": "imc_pediatre", "resultat": result,
            "moteur": "medisuite_rules.scores"}
