"""Tests nuclear-medicine-service : santé, module-info, scores, cas cliniques."""
import pathlib
import sys

ROOT = pathlib.Path(__file__).resolve().parents[3]
for p in ("packages/medisuite-core", "packages/clinical-rules",
          str(ROOT / "services" / "nuclear-medicine-service" / "src")):
    sys.path.insert(0, p)
...
def test_module_info():
    ...
    assert info["module"] == 22 and "nuclear-medicine-service" in info["service"]
...
def test_score_suv():
    r = client.post("/api/v1/scores/suv", json={'activite_kbq_ml': 6.5, 'dose_injectee_MBq': 250, 'poids_kg': 72}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide


def test_score_ecog():
    r = client.post("/api/v1/scores/ecog", json={'karnofsky': 90}, headers=HDR)
    assert r.status_code == 200
    assert r.json()["resultat"]  # résultat structuré non vide
