"""Fertilité (🧬) — Module 12 MEDISUITE.

Réserve ovarienne AMH/CFA, spermogramme OMS 2021, sélection embryonnaire, FIV.
...
app: FastAPI = create_service_app(
    "fertility-service", "Fertilité", "Réserve ovarienne AMH/CFA, spermogramme OMS 2021, sélection embryonnaire, FIV.",
    module_label="Module 12 · Fertilité")

engine = engine_for("fertility-service")
...
def seed() -> None:
    with SessionLocal() as db:
        if db.scalar(select(Case).limit(1)):
            return
        demo = [('Bilan infertilité couple', 'pat0028', 'WATAT Sandra', 'AMH + CFA'), ('Spermogramme contrôle', 'pat0029', 'KOFFI Jean-Marc', 'OMS 2021'), ('Ponction FIV J0', 'pat0030', 'ADOU Béatrice', '8 ovocytes')]
...
def module_info() -> dict:
    return {"module": 12, "nom": "Fertilité", "service": "fertility-service",
            "scores": ['roma'],
