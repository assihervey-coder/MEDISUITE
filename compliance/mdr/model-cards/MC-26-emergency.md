# MC-26 — Urgences — triage (emergency)
...
| Service | `emergency-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/26_emergency.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : ESI/CTMP/Manchester, qSOFA, ISS, NIHSS code AVC, STEMI, Parkland.

**Ce que la sortie IA influence** : catégorie ESI → priorité de prise en charge ; réévaluation clinique systématique.
...
| Tâche | `multiclass` — multiclasse — `hors_danger`, `urgent`, `tres_urgent`, `vital` (poids [0.35, 0.3, 0.2, 0.15]) |
| Features cliniques nommées | `esi`, `qsofa`, `iss`, `nihss`, `shock_index` |
| Variables discriminantes (signal artificiel) | `qsofa`, `shock_index` |
...
données `docs/_audit_data.json`) : 202 lignes de service, 9 tests,
