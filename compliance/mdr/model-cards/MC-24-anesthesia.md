# MC-24 — Anesthésie — événement péri-opératoire (anesthesia)
...
| Service | `anesthesia-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/24_anesthesia.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : ASA, Lee/RCRI, STOP-BANG, SOFA/APACHE II, RASS, analgésie multimodale.

**Ce que la sortie IA influence** : risque péri-opératoire → orientation de la consultation pré-anesthésique.
...
| Tâche | `classification` — binaire — `evenement_perop` (prévalence synthétique 0.18) |
| Features cliniques nommées | `asa`, `lee_rcri`, `stopbang`, `sofa`, `duree_chir_h` |
| Variables discriminantes (signal artificiel) | `asa`, `sofa` |
...
| Modalités attendues | `signal_1d`, `tabulaire`, `waveform` |
...
données `docs/_audit_data.json`) : 164 lignes de service, 6 tests,
