# MC-13 — Neurologie — AVC ischémique (neurology)
...
| Service | `neurology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/13_neurology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : AVC (ASPECTS, NIHSS), épilepsie EEG, MMSE/MoCA, Parkinson UPDRS, McDonald/EDSS.

**Ce que la sortie IA influence** : AVC ischémique probable → activation du code AVC et fenêtre de thrombolyse ; l'horloge LKW est VALIDÉE par le clinicien (RM-10).
...
| Tâche | `classification` — binaire — `avc_ischemique` (prévalence synthétique 0.4) |
| Features cliniques nommées | `nihss`, `aspects`, `mmse`, `age`, `delai_onset_h` |
| Variables discriminantes (signal artificiel) | `nihss`, `aspects` |
...
| Modalités attendues | `imaging_3d`, `tabulaire`, `texte` |
...
Accentuations spécifiques du module : RM-07, RM-10.
