# MC-03 — Oncologie — stadification tumorale (oncology)
...
| Service | `oncology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/03_oncology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Dépistage (7 organes), BI-RADS, TNM/AJCC 8e, biomarqueurs, RCP.

**Ce que la sortie IA influence** : classe de malignité suspectée → priorisation en RCP ; décision thérapeutique multidisciplinaire.
...
| Tâche | `multiclass` — multiclasse — `sain`, `benin`, `invasif_precoce`, `invasif_avance` (poids [0.4, 0.3, 0.2, 0.1]) |
| Features cliniques nommées | `ca_125_u_ml`, `psa_ng_ml`, `taille_tumeur_mm`, `ecog`, `age` |
| Variables discriminantes (signal artificiel) | `ca_125_u_ml`, `taille_tumeur_mm` |
...
| Modalités attendues | `imaging_2d`, `imaging_3d`, `tabulaire`, `texte`, `genomique` |
...
données `docs/_audit_data.json`) : 201 lignes de service, 9 tests,
6 scores cliniques implémentés (référentiel cité par score).
...
Accentuations spécifiques du module : RM-02.
...
| Tests verrouillants | 9 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
