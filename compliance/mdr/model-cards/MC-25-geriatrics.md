# MC-25 — Gériatrie — risque de chute (geriatrics)
...
| Service | `geriatrics-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/25_geriatrics.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Fried, Rockwood, TUG, MMSE/MoCA, Beers/STOPP-START, MNA, Braden, Barthel.

**Ce que la sortie IA influence** : risque de chute à 12 mois → programme de prévention (revue Beers, environnement).
...
| Tâche | `classification` — binaire — `chute_12_mois` (prévalence synthétique 0.3) |
| Features cliniques nommées | `fried`, `rockwood`, `tug_s`, `mna`, `beers_n`, `braden` |
| Variables discriminantes (signal artificiel) | `fried`, `tug_s` |
...
données `docs/_audit_data.json`) : 188 lignes de service, 8 tests,
5 scores cliniques implémentés (référentiel cité par score).
...
| Tests verrouillants | 8 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
