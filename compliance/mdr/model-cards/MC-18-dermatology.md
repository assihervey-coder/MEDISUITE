# MC-18 — Dermatologie — lésions pigmentaires (dermatology)
...
| Service | `dermatology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/18_dermatology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Mélanome ABCDE/7-points, Breslow, PASI, SCORAD, VASI.

**Ce que la sortie IA influence** : naevus / douteux / mélanome → biopsie orientée ; le Breslow histologique reste la référence.
...
| Tâche | `multiclass` — multiclasse — `nevus`, `douteux`, `melanome` (poids [0.5, 0.3, 0.2]) |
| Features cliniques nommées | `diametre_mm`, `asymetrie`, `couleurs_n`, `breslow_mm`, `evolutivite` |
| Variables discriminantes (signal artificiel) | `breslow_mm`, `asymetrie` |
...
| Modalités attendues | `tabulaire`, `imaging_2d`, `texte` |
...
données `docs/_audit_data.json`) : 162 lignes de service, 6 tests,
3 scores cliniques implémentés (référentiel cité par score).
...
| Tests verrouillants | 6 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
