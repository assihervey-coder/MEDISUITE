# MC-15 — Pédiatrie — croissance (pediatrics)
...
| Service | `pediatrics-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/15_pediatrics.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : APGAR, courbes OMS, ictère néonatal, calendrier vaccinal PEV, Denver.

**Ce que la sortie IA influence** : indice de croissance → dépistage de la malnutrition (courbes OMS).
...
| Tâche | `regression` — régression — `indice_croissance` ∈ [0.0, 1.0] |
| Features cliniques nommées | `poids_z`, `taille_z`, `apgar`, `bilirubine_mg_dl`, `age_mois` |
| Variables discriminantes (signal artificiel) | `poids_z`, `taille_z` |
...
données `docs/_audit_data.json`) : 209 lignes de service, 5 tests,
