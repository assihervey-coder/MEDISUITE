# MC-14 — Psychiatrie — sévérité globale (psychiatry)
...
| Service | `psychiatry-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/14_psychiatry.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : PHQ-9, GAD-7, Y-BOCS, C-SSRS risque suicidaire, AUDIT/DAST.

**Ce que la sortie IA influence** : sévérité → intensification du suivi ; un C-SSRS élevé déclenche la conduite clinique immédiate, hors IA.
...
| Tâche | `regression` — régression — `severite_globale` ∈ [0.0, 30.0] |
| Features cliniques nommées | `phq9`, `gad7`, `audit`, `sommeil_h`, `cssrs` |
| Variables discriminantes (signal artificiel) | `phq9`, `gad7` |
