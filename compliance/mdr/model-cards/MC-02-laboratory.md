# MC-02 — Laboratoire — sévérité biologique (laboratory)
...
| Service | `laboratory-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/02_laboratory.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Prescription → prélèvement → analyse → validation (Westgard) → compte-rendu.

**Ce que la sortie IA influence** : triage des dossiers biologiques critiques ; la validation reste au biologiste.
...
| Tâche | `regression` — régression — `score_severite_0_10` ∈ [0.0, 10.0] |
| Features cliniques nommées | `hgb_g_dl`, `wbc_g_l`, `plt_g_l`, `creatinine_mg_l`, `glycemie_g_l`, `crp_mg_l` |
| Variables discriminantes (signal artificiel) | `crp_mg_l`, `wbc_g_l` |
...
| Modalités attendues | `tabulaire`, `texte` |
...
données `docs/_audit_data.json`) : 267 lignes de service, 8 tests,
...
Accentuations spécifiques du module : RM-08.
