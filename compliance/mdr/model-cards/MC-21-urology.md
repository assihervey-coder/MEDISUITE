# MC-21 — Urologie — cancer de prostate significatif (urology)
...
| Service | `urology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/21_urology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : PI-RADS, Gleason, IPSS, score RENAL, calculs (SWL/URS).

**Ce que la sortie IA influence** : cancer significatif probable → biopsies ciblées (PI-RADS).
...
| Tâche | `classification` — binaire — `ca_prostate_significatif` (prévalence synthétique 0.28) |
| Features cliniques nommées | `pirads`, `ips`, `psa_ng_ml`, `gleason`, `renal_score` |
| Variables discriminantes (signal artificiel) | `pirads`, `gleason` |
