# MC-16 — Néphrologie — stade d'insuffisance rénale (nephrology)
...
| Service | `nephrology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/16_nephrology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : CKD-EPI/KDIGO, AKI KDIGO, kt/V dialyse, néphrométrie RENAL.

**Ce que la sortie IA influence** : stade MRC → référencement néphrologue ; kt/V oriente la dose de dialyse.
...
| Tâche | `regression` — régression — `stade_ckd_1_5` ∈ [1.0, 5.0] |
| Features cliniques nommées | `creatinine_mg_l`, `dfao_ckdepi`, `kaliemia_mmol_l`, `proteinurie_g_j`, `ktv` |
| Variables discriminantes (signal artificiel) | `dfao_ckdepi`, `creatinine_mg_l` |
