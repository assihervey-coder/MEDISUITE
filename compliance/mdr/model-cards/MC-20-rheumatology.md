# MC-20 — Rhumatologie — activité inflammatoire (rheumatology)
...
| Service | `rheumatology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/20_rheumatology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : DAS28, BASDAI/ASDAS, SLEDAI, Kellgren-Lawrence, critères ANCA.

**Ce que la sortie IA influence** : activité → proposition d'ajustement thérapeutique au rhumatologue.
...
| Tâche | `regression` — régression — `activite_0_10` ∈ [0.0, 10.0] |
| Features cliniques nommées | `das28`, `crp_mg_l`, `facteur_rhumato_ui`, `sledai`, `kl_grade` |
| Variables discriminantes (signal artificiel) | `das28`, `crp_mg_l` |
