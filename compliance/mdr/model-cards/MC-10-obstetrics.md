# MC-10 — Obstétrique — pré-éclampsie (obstetrics)
...
| Service | `obstetrics-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/10_obstetrics.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Biométrie fœtale, CTG/FIGO, clarté nucale, pré-éclampsie, Bishop.

**Ce que la sortie IA influence** : risque de pré-éclampsie → surveillance rapprochée mère-foetus.
...
| Tâche | `classification` — binaire — `pre_eclampsie` (prévalence synthétique 0.2) |
| Features cliniques nommées | `tas_mmhg`, `proteinurie_g_j`, `clarte_nucale_mm`, `bishop`, `age_gestation_sem` |
| Variables discriminantes (signal artificiel) | `tas_mmhg`, `proteinurie_g_j` |
...
| Modalités attendues | `waveform`, `imaging_2d`, `tabulaire` |
