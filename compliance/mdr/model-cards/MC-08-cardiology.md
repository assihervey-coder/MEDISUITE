# MC-08 — Cardiologie — syndrome coronarien (cardiology)
...
| Service | `cardiology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/08_cardiology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : ECG 12 dérivations, ETT (FEVG), coro-CT CAD-RADS, Holter, CHA2DS2-VASc.

**Ce que la sortie IA influence** : stable / angor instable / SCA → filière coronarienne ; l'ECG reste l'élément décisionnel premier.
...
| Tâche | `multiclass` — multiclasse — `stable`, `angor_instable`, `sca` (poids [0.55, 0.25, 0.2]) |
| Features cliniques nommées | `fevg_pct`, `troponine_ng_ml`, `bpm`, `qt_ms`, `cha2ds2_vasc` |
| Variables discriminantes (signal artificiel) | `troponine_ng_ml`, `fevg_pct` |
...
| Modalités attendues | `signal_1d`, `tabulaire`, `texte` |
...
données `docs/_audit_data.json`) : 175 lignes de service, 7 tests,
4 scores cliniques implémentés (référentiel cité par score).
...
Accentuations spécifiques du module : RM-07.
...
| Tests verrouillants | 7 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
