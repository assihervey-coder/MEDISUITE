# MC-23 — Radiothérapie — toxicité prédite (radiotherapy)
...
| Service | `radiotherapy-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/23_radiotherapy.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : GTV/CTV/PTV, IMRT/VMAT, DVH, gamma index, EQD2, QA machine.

**Ce que la sortie IA influence** : toxicité prédite → optimisation de planification (DVH) ; la QA machine reste indépendante.
...
| Tâche | `regression` — régression — `toxicite_0_1` ∈ [0.0, 1.0] |
| Features cliniques nommées | `dose_gy`, `fractions`, `v20_pct`, `gamma_pct`, `eqd2_gy` |
| Variables discriminantes (signal artificiel) | `v20_pct`, `eqd2_gy` |
