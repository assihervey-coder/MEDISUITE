# MC-09 — Pneumologie — TB / BPCO / embolie (pneumology)
...
| Service | `pneumology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/09_pneumology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Nodules (Fleischner/Lung-RADS), TB, BPCO GOLD, embolie (Wells), spirométrie.

**Ce que la sortie IA influence** : signaux multilabel → examens complémentaires (imagerie, D-dimères, spirométrie).
...
| Tâche | `multilabel` — multilabel — `tb_active`, `bpco`, `embolie` (p(ind)≈[0.15, 0.35, 0.2]) |
| Features cliniques nommées | `spo2_pct`, `fev1_pct`, `wells`, `crp_mg_l`, `paquets_annees` |
| Variables discriminantes (signal artificiel) | `spo2_pct`, `fev1_pct` |
