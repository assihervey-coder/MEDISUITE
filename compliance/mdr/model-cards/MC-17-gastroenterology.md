# MC-17 — Gastro-entérologie — hémorragie active (gastroenterology)
...
| Service | `gastroenterology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/17_gastroenterology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Polypes, LI-RADS, Mayo/CDEIS (MICI), Los Angeles (RGO), Forrest.

**Ce que la sortie IA influence** : hémorragie active probable → endoscopie urgente (classification Forrest).
...
| Tâche | `classification` — binaire — `hemorragie_active` (prévalence synthétique 0.3) |
| Features cliniques nommées | `forrest`, `li_rads`, `hb_g_dl`, `alcool_u_sem`, `nodule_mm` |
| Variables discriminantes (signal artificiel) | `forrest`, `hb_g_dl` |
