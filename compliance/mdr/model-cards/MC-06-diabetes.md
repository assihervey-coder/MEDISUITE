# MC-06 — Diabète — risque de complication (diabetes)
...
| Service | `diabetes-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/06_diabetes.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Risque ADA, HbA1c/TIR, complications, CGM (Dexcom/Libre/Medtronic).

**Ce que la sortie IA influence** : intensification du suivi ; aucun ajustement thérapeutique autonome.
...
| Tâche | `regression` — régression — `risque_complication` ∈ [0.0, 1.0] |
| Features cliniques nommées | `hba1c_pct`, `tir_pct`, `age_dx`, `imc`, `fbg_g_l` |
| Variables discriminantes (signal artificiel) | `hba1c_pct`, `tir_pct` |
...
données `docs/_audit_data.json`) : 150 lignes de service, 5 tests,
