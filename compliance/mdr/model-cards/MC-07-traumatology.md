# MC-07 — Traumatologie — polytrauma (traumatology)
...
| Service | `traumatology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/07_traumatology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Fractures AO/OTA, ligaments (LCA…), Cobb, FRAX, polytrauma ISS.

**Ce que la sortie IA influence** : fracture ouverte / atteinte viscérale / chirurgie requise → priorisation du déchoquage.
...
| Tâche | `multilabel` — multilabel — `fracture_ouverte`, `atteinte_viscerale`, `chirurgie_requise` (p(ind)≈[0.2, 0.3, 0.4]) |
| Features cliniques nommées | `iss`, `gcs`, `age`, `fractures_n`, `cobb_deg` |
| Variables discriminantes (signal artificiel) | `iss`, `fractures_n` |
