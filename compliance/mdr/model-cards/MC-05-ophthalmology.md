# MC-05 — Ophtalmologie — rétinopathie / glaucome (ophthalmology)
...
| Service | `ophthalmology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/05_ophthalmology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Fond d'œil, OCT, glaucome (C/D, RNFL), rétinopathie diabétique.

**Ce que la sortie IA influence** : stade de rétinopathie diabétique → calendrier de dépistage ; urgences orientées en ophtalmologie.
...
| Tâche | `multiclass` — multiclasse — `rd_absente`, `rd_non_proliferante`, `rd_proliferante` (poids [0.5, 0.3, 0.2]) |
| Features cliniques nommées | `cd_ratio`, `rnfl_um`, `acuite_logmar`, `hba1c_pct`, `pio_mmhg` |
| Variables discriminantes (signal artificiel) | `cd_ratio`, `rnfl_um` |
...
| Modalités attendues | `imaging_2d`, `tabulaire` |
...
données `docs/_audit_data.json`) : 149 lignes de service, 5 tests,
2 scores cliniques implémentés (référentiel cité par score).
...
Accentuations spécifiques du module : aucune au-delà du socle commun.
...
| Tests verrouillants | 5 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
