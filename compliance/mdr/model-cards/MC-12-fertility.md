# MC-12 — Fertilité — pronostic de conception (fertility)
...
| Service | `fertility-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/12_fertility.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Réserve ovarienne (AMH/CFA), spermogramme OMS 2021, sélection embryonnaire.

**Ce que la sortie IA influence** : pronostic → stratégie d'AMP ; décision collégiale.
...
| Tâche | `regression` — régression — `proba_conception` ∈ [0.0, 1.0] |
| Features cliniques nommées | `amh_ng_ml`, `cfa_n`, `spermato_millions`, `fsh_ui_l`, `age_femme` |
| Variables discriminantes (signal artificiel) | `amh_ng_ml`, `spermato_millions` |
...
données `docs/_audit_data.json`) : 136 lignes de service, 4 tests,
1 scores cliniques implémentés (référentiel cité par score).
...
| Tests verrouillants | 4 tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
