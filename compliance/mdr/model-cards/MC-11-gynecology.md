# MC-11 — Gynécologie — malignité suspectée (gynecology)
...
| Service | `gynecology-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/11_gynecology.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Pap/Bethesda, HPV, IOTA simple rules, O-RADS, SOPK Rotterdam, ENZIAN.

**Ce que la sortie IA influence** : malignité suspectée → orientation échographie dédiée / RCP gynécologique.
...
| Tâche | `classification` — binaire — `malignite_suspectee` (prévalence synthétique 0.25) |
| Features cliniques nommées | `pap_bethesda`, `hpv_vl_log`, `iota_score`, `orads`, `saignement_j` |
| Variables discriminantes (signal artificiel) | `iota_score`, `orads` |
