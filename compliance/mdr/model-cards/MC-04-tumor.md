# MC-04 — Tumeurs cérébrales — volumétrie (tumor)
...
| Service | `tumor-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/04_tumor.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Segmentation cérébrale, volumétrie, grading OMS 2021, planification RT.

**Ce que la sortie IA influence** : fraction lésionnelle → suivi volumétrique ; le contouring seul ne suffit pas à planifier la RT.
...
| Tâche | `segmentation` — segmentation — `fraction_lesionnelle` ∈ [0.0, 0.6] |
| Features cliniques nommées | `volume_tumeur_mm3`, `oedema_mm`, `grade_oms`, `karnofsky` |
| Variables discriminantes (signal artificiel) | `volume_tumeur_mm3`, `oedema_mm` |
...
Métriques prévues (segmentation) :
