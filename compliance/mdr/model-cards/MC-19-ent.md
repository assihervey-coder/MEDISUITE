# MC-19 — ORL — tumeur suspectée (ent)
...
| Service | `ent-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/19_ent.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : Audiogramme, BPPV, Lund-Mackay, cancers ORL (HPV).

**Ce que la sortie IA influence** : tumeur ORL suspectée → imagerie et biopsie orientées.
...
| Tâche | `classification` — binaire — `tumeur_orl_suspectee` (prévalence synthétique 0.2) |
| Features cliniques nommées | `perte_db`, `lund_mackay`, `pta_db`, `tinnitus`, `hpv_pos` |
| Variables discriminantes (signal artificiel) | `perte_db`, `lund_mackay` |
