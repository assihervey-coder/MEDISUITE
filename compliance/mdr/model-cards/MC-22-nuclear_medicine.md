# MC-22 — Médecine nucléaire — réponse thérapeutique (nuclear_medicine)
...
| Service | `nuclear-medicine-service` (monorepo `services/`) |
| Config IA | `ai/multimodal/configs/22_nuclear_medicine.yaml` |
| État | 🟢 structure · 🟠 données synthétiques · 🔴 métriques (R6-R8) · CER TD-11 à compléter R7 |

## 1. Usage prévu (intended purpose du module)

**Périmètre fonctionnel** : SUV, MTV/TLG, PERCIST, dosimétrie MIRD, théranostique Lu-177.

**Ce que la sortie IA influence** : pourcentage de réponse (PERCIST) → poursuite ou modification de la théranostique.
...
| Tâche | `regression` — régression — `reponse_pct` ∈ [-40.0, 100.0] |
| Features cliniques nommées | `suv_max`, `mtv_cm3`, `tlg`, `dose_mbq`, `dlco_pct` |
| Variables discriminantes (signal artificiel) | `suv_max`, `mtv_cm3` |
| Volumes synthétiques | train n/a / val (cf. `datasets/manifest.json`, SHA-256 vérifiés par tests) |
...
| Modalités attendues | `imaging_3d`, `tabulaire` |
...
données `docs/_audit_data.json`) : ? lignes de service, ? tests,
? scores cliniques implémentés (référentiel cité par score).
...
Accentuations spécifiques du module : RM-04.
...
| Tests verrouillants | ? tests du service ; datasets (schéma/déterminisme/signal) ; IA 47 tests |
